...
class Solver1D : public CuriouslyRecurringTemplate<Impl> {
  public:
    Solver1D()
        : maxEvaluations_(MAX_FUNCTION_EVALUATIONS), lowerBoundEnforced_(false),
    ...
    */
    ...
    T solve(const F &f, T accuracy, T guess, T xMin, T xMax) const {
        ...
        QL_REQUIRE(!lowerBoundEnforced_ || xMin_ >= lowerBound_,
                   "xMin_ (" << xMin_ << ") < enforced low bound ("
                             << lowerBound_ << ")");
        QL_REQUIRE(!upperBoundEnforced_ || xMax_ <= upperBound_,
                   "xMax_ (" << xMax_ << ") > enforced hi bound ("
                             << upperBound_ << ")");
    ...
    */
    ...
    //@}
  protected:
    mutable T root_, xMin_, xMax_, fxMin_, fxMax_;
    Size maxEvaluations_;
    mutable Size evaluationNumber_;

  private:
    T enforceBounds_(T x) const;
...
inline void Solver1D<Impl, T>::setMaxEvaluations(Size evaluations) {
    maxEvaluations_ = evaluations;
}

template <class Impl, class T>
inline void Solver1D<Impl, T>::setLowerBound(T lowerBound) {
    lowerBound_ = lowerBound;
    lowerBoundEnforced_ = true;
}

template <class Impl, class T>
inline void Solver1D<Impl, T>::setUpperBound(T upperBound) {
    upperBound_ = upperBound;
    upperBoundEnforced_ = true;
}

template <class Impl, class T>
inline T Solver1D<Impl, T>::enforceBounds_(T x) const {
