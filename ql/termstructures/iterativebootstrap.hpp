...
template <class Curve, class T = Real> class IterativeBootstrap {
  ...
  private:
    ...
    Curve *ts_;
    Size n_;
    Brent_t<T> firstSolver_;
    FiniteDifferenceNewtonSafe_t<T> solver_;
    mutable bool initialized_, validCurve_;
    mutable Size firstAliveHelper_, alive_;
    mutable std::vector<T> previousData_;
    mutable std::vector<boost::shared_ptr<BootstrapError<Curve, T> > > errors_;
};

// template definitions

template <class Curve, class T>
IterativeBootstrap<Curve, T>::IterativeBootstrap()
    : ts_(0), initialized_(false), validCurve_(false) {}

template <class Curve, class T>
void IterativeBootstrap<Curve, T>::setup(Curve *ts) {
...
template <class Curve, class T>
void IterativeBootstrap<Curve, T>::initialize() const {
    ...
    for (Size i = 1, j = firstAliveHelper_; j < n_; ++i, ++j) {
        ...
        errors_[i] = boost::shared_ptr<BootstrapError<Curve, T> >(
            new BootstrapError<Curve, T>(ts_, helper, i));
    }
    ...
    if (!validCurve_ || ts_->data_.size() != alive_ + 1) {
        ...
        ts_->data_ = std::vector<T>(alive_ + 1, Traits::initialValue(ts_));
...
template <class Curve, class T>
void IterativeBootstrap<Curve,T>::calculate() const {
    ...
    for (Size j = firstAliveHelper_; j < n_; ++j) {
        ...
        QL_REQUIRE(helper->quote()->isValid(),
                   io::ordinal(j + 1)
                       << " instrument (maturity: " << helper->latestDate()
    ...
    const std::vector<T> &data = ts_->data_;
    T accuracy = ts_->accuracy_;
    ...
    for (Size iteration = 0;; ++iteration) {
        ...
        for (Size i = 1; i <= alive_; ++i) { // pillar loop
            ...
            T min = Traits::minValueAfter(i, ts_, validData, firstAliveHelper_);
            T max = Traits::maxValueAfter(i, ts_, validData, firstAliveHelper_);
            T guess = Traits::guess(i, ts_, validData, firstAliveHelper_);
            ...
            if (!validData) {
                try { // extend interpolation a point at a time
                    ...
                    ts_->interpolation_ = Linear<T>().interpolate(
                        times.begin(), times.begin() + i + 1, data.begin());
                }
        ...
        QL_REQUIRE(iteration < maxIterations,
                   "convergence not reached after "
                       << iteration << " iterations; last improvement "
                       << change << ", required accuracy " << accuracy);
    }
    validCurve_ = true;
}
}
