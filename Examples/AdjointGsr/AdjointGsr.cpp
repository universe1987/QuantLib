...
int main() {
    ...
    try {
        ...
        std::vector<std::vector<Handle<Quote_t<dbl> > > > volSpreads,
            sabrParams;
        ...
        boost::shared_ptr<SwapIndex_t<dbl> > indexBase =
            boost::make_shared<EuriborSwapIsdaFixA_t<dbl> >(30 * Years,
                                                            yts6m_h);
        ...
        boost::shared_ptr<VanillaSwap_t<dbl> > underlying =
            boost::make_shared<VanillaSwap_t<dbl> >(
                VanillaSwap_t<dbl>::Payer, 1.0, fixedSchedule, strike,
                Thirty360(), floatingSchedule, euribor6m, 0.0, Actual360());
        ...
        boost::shared_ptr<Gsr_t<dbl> > gsr = boost::make_shared<Gsr_t<dbl> >(
            yts6m_h, stepDates, sigmas, reversion);
        boost::shared_ptr<Gsr_t<dblAD> > gsrAD =
            boost::make_shared<Gsr_t<dblAD> >(yts6mAD_h, stepDates, sigmasAD,
                                              reversion);
        ...
        boost::shared_ptr<PricingEngine> swaptionEngineAD =
            boost::make_shared<Gaussian1dSwaptionEngine_t<dblAD> >(gsrAD, 32,
                                                                   5.0);
        ...
        for (Size i = 0; i < basket.size(); ++i) {
            boost::shared_ptr<SwaptionHelper_t<dblAD> > helper =
                boost::dynamic_pointer_cast<SwaptionHelper_t<dblAD> >(
                    basketAD[i]);
            std::cout << helper->swaption()->exercise()->date(0) << ";"
                      << volatilityAD[i] << ";" << helper->modelValue() << ";"
                      << helper->marketValue() << ";" << inputVolAD[i]
                      << std::endl;
        }
        std::cout << std::endl;
        ...
        std::cout << "plain pricing bermudan npv =" << swaption->NPV()
                  << std::endl;
        timer.stop();
        ...
        trace_off();
#endif
        std::cout << "AD pricing bermudan npv = " << std::setprecision(12)
                  << yAD[0] << std::endl;
        timer.stop();
